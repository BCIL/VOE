function D3LineChart(options) {
	...
	var defaultOptions = {
		...
		tooltipOnMouseOver:	function(d, element, base) {
			...
			if (a != null){
				...
				if(chr_chk != 'null') {
					...
					/// shows table below linechart
					//renderPositionTable(begin,end);
				}
			}
		},
		...
		renderTableMouseClick: function (d, element, base) {
			...
			var xPosition = parseInt($(element).attr('cx'))-base.tooltipWidth/2+base.options.margin.left+300;
			var yPosition = base.y.range()[0]-parseInt($(element).attr('cy'))+base.options.margin.bottom+300;
			$(".tooltip").empty();
			d3.select(base.options.container+" .tooltip")
				.style("left", xPosition + "px")
				.style("bottom", yPosition + "px")	
				.html("The table is generated!");
			$(base.options.container+" .tooltip").show();
			$(element).parent().find("path").css("opacity", 1);
			setTimeout(function() {
