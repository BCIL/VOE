...
nv.models.axis = function() {
    ...
    function chart(selection) {
        renderWatch.reset();
        selection.each(function(data) {
            window.test_selection_data = data;
            var container = d3.select(this);
            ...
            window.fmt = axis.tickFormat();
            //console.log("fmt: " + fmt)
            if (fmt == null) {
                fmt = scale0.tickFormat();
                //console.log("fmt(if-null): " + fmt)
            }
                ...
                case 'left':
                    ...
                    if (showMaxMin) {
                        ...
                        axisMaxMin
                            ...
                            .text(function(d,i) {
                                var v = fmt(d);
                                //console.log("y min-max: " + v + " // data: "+ d + " // index: " + i)
                                return ('' + v).match('NaN') ? '' : v;
                            });
                        axisMaxMin.watchTransition(renderWatch, 'min-max right')
                            .attr('transform', function(d,i) {
...
nv.models.ohlcBarChart = function() {
    ...
    function chart(selection) {
        if(typeof(chart_selection) === 'undefined'){
           window.chart_selection = [];
        }
        chart_selection.push(selection)
        selection.each(function(data) {
            var availableWidth = width - margin.left - margin.right,
                container = d3.select(this);
            nv.utils.initSVG(container);
            ...
            var series = g.selectAll('.nv-series')
                .data(function(d) {window.test_d = d; return d });
            //debugger;
...
nv.models.lineWithFocusChart = function() {
    ...
    var showTooltip = function(e, offsetElement) {
        var left = e.pos[0] + ( offsetElement.offsetLeft || 0 ),
            ...
            content = tooltip(e.series.key, x, y, e, chart);
            //window.cont = content;
            //window.tool_pos = [left, top]
            window.tooltip_e = e;
            //window.offsetElement = offsetElement;
        nv.tooltip.show([left, top], content, null, null, offsetElement);
    };
    ...
    function chart(selection) {
        selection.each(function(data) {
            ...
            y2Axis
                ...
                .tickSize( -availableWidth, 0);
            window.test_y2Axis = y2Axis.scale(y2);
            window.test_height1 = availableHeight1;
            window.test_height2 = availableHeight2;
...
nv.models.multiBarChart = function() {
    ...
    var margin = {top: 30, right: 20, bottom: 50, left: 60}
        ...
        , rotateLabels = 0
        , tooltips = true
        , tooltip = function(key, x, y, gene, e, graph) {
            window.test_e = e;
            window.test_gene = gene;
            return '<h3>' + key + '</h3>' +
                '<p> gene: ' + gene.series.values[gene.pointIndex].gene+ '</p>' +
                '<p> value: ' +  y + '</p>' +
                '<p> index: ' +  x + '</p>'
        }
        , x //can be accessed via chart.xScale()
    ...
    var showTooltip = function(e, offsetElement) {
        window.showtooltip_multiBar = {};
        showtooltip_multiBar.e = e;
        showtooltip_multiBar.offsetElement = offsetElement;
        var left = e.pos[0] + ( offsetElement.offsetLeft),
...
nv.models.pieChart = function() {
    ...
    var margin = {top: 30, right: 20, bottom: 20, left: 20}
        ...
        , tooltip = function(key, y, e, graph) {
            if(typeof(donut_data)) {

                var p = (e.value / donut_data.total) * 100;
                //console.log(e.value + " // total: " + donut_data.total)
                $("#pieChart-title").empty();
                $("#pieChart-title").append(parseFloat(p).toFixed(2) + "%");   
            }
        ...
        , noData = "No Data Available."
        , duration = 800
        , dispatch = d3.dispatch('tooltipShow', 'tooltipHide','chartClick', 'elementClick', 'stateChange', 'changeState','renderEnd')
        ;
    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            pie.dispatch.on('elementClick', function(e) {
                ...
                if (typeof(input_data_ready)) {
/////////////   
// Clicks pie graph element -> generates a custom dataset then displays new visualization
////////////                
                    gen_custom_dataset(e);
                }
            })
